...
texture Src03<string uiname="Source 3";>;
sampler Src03Samp = sampler_state    //sampler for doing the texture-lookup
{
    Texture   = (Src03);          //apply a texture to the sampler
    MipFilter = LINEAR;         //sampler states
    MinFilter = LINEAR;
    MagFilter = LINEAR;
    AddressU = Mirror;
...
//mask transformation
float4x4 tSrc03: TEXTUREMATRIX <string uiname="Source 3 Transform";>;
...
float Src03Alpha
<
    string uiname="Source 3 Transparency";
    float ui3min=0.0;
    float ui3max=1.0;
> = 0.0;
    ...
    float4 Src03Cd : TEXCOORD2)
{
    ...
    //transform the mask texture cordinates
    Out.Src03Cd = mul(Src01Cd, tSrc03);
...
{
    float4 tempColor = { 0.0, 0.0, 0.0, 1.00000 };
    ...
    // the mask texture lookup
    float4 src03 = tex2D(Src03Samp, In.Src03Cd);

    tempColor = lerp(src02, src01, 1-Src02Alpha);
    outColor = lerp(src03, tempColor, 1-Src03Alpha);
 ...
 float4 psScreen(vs2ps In):Color
{
    ...
    tempColor = lerp(src02, src01, 1-Src02Alpha);
    src03 = 1.0 - (1.0-(tempColor)) * (1.0-(src03));
    outColor = lerp(src03, tempColor, 1-Src03Alpha);
